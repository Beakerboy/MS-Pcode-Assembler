...
class ProjectCache():
    ...
    def _data_section(self: T) -> bytes:
        ...
        for num in data[8:20]:
            ca += struct.pack("<h", num)
        ca += struct.pack("<iihhi", -1, -1, data[20], data[21], -1)
        for num in data[22:26]:
            ca += struct.pack("<h", num)
        # Footer 22 bytes
        ...
        for name in names:
            ...
            if junk_count < junk_ids:
                if type & 0x80:
                    ...
                    data2 = name[4]
                    ca += (struct.pack(
                        "<HHBBHbhb", 0, data, len(name[0]), type, data1,
                        -1 if len(name) < 6 else name[5],
                        data2, 0 if data2 >= 0 else -1
                    ) + name[0])
                else:
                ...
                if type & 0x80:
                    ...
                    data2 = name[4]
                    ca += (struct.pack(
                        "<BBHbhb", len(name[0]), type, data1,
                        -1 if len(name) < 6 else name[5], data2,
                        0 if data2 >= 0 else -1
    ...
    def _footer_section(self: T) -> bytes:
        length = len(self._footer[0])
        data0 = self._footer[0]
        data1 = self._footer[1]
        return struct.pack("<BhHI", 2, -1, 0x101, length) + data0 + data1
